...
test("homepage stays within the repo-specific performance budget", async ({ page }) => {
  ...
  expect(stylesheet?.encodedBodySize ?? Number.POSITIVE_INFINITY).toBeLessThan(20_000);
  expect(heroImage).toBeDefined();
  expect(heroImage?.encodedBodySize ?? Number.POSITIVE_INFINITY).toBeLessThan(60_000);
  expect(performanceSummary.resources.length).toBeLessThanOrEqual(3);
